...
void VSMain(const VSInput input, out PSInput output)
{
	...
	output.originalPos = mul(input.pos, g_W);
...
void PSMain(const PSInput input, out PSOutput output)
{
	...
	float3 perspectiveCorrection = lightSpace.xyz / lightSpace.w;

	// Scale and offset uvs into 0-1 range
	float2 perspectiveUV = (perspectiveCorrection.xy * 0.5f) + 0.5f;

	// Sample render target to see if this pixel is in shadow
	float4 shadowSample = g_shadowTexture.Sample(g_shadowSampler, perspectiveUV);

	float4 interpolatedShadow = lerp(shadowSample, g_shadowColour, 0.0f);

	// If it is then alpha blend between final colour and shadow colour
	output.colour = input.colour * (1.0f - interpolatedShadow.r);
}